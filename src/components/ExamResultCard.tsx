...
export default function ExamResultCard({ 
  ...
  // Calculate inspection scores based on Conforme, No Conforme, and No Aplica (N/A)
  const conformeCount = answers.filter(a => {
    const lower = (a.elegida || "").toLowerCase();
    return lower.includes("conforme") && !lower.includes("no conforme");
  }).length;

  const noConformeCount = answers.filter(a => {
    const lower = (a.elegida || "").toLowerCase();
    return lower.includes("no conforme");
  }).length;

  const noAplicaCount = answers.filter(a => {
    const lower = (a.elegida || "").toLowerCase();
    return lower.includes("no aplica") || lower.includes("n/a");
  }).length;

  const totalEvaluadas = conformeCount + noConformeCount;
  const score = totalEvaluadas > 0 ? Math.round((conformeCount / totalEvaluadas) * 100) : 100;
  const isApproved = score >= 80;

  const bancoUsado = (registration.tipoLicencia || "").toLowerCase().includes("moto") ? "Banco 1" : "Banco 2";
  ...
  const finalResultPayload: ExamResult = {
    ...
    edad: registration.edad !== undefined && registration.edad !== "" ? Number(registration.edad) : 0,
    empresa: registration.empresa,
    antiguedad: registration.antiguedad !== undefined && registration.antiguedad !== "" ? Number(registration.antiguedad) : 0,
    tipoLicencia: registration.tipoLicencia as any,
    bancoUsado: bancoUsado,
    conformeCount: conformeCount,
    noConformeCount: noConformeCount,
    noAplicaCount: noAplicaCount,
    totalEvaluadas: totalEvaluadas,
    correctas: conformeCount,
    incorrectas: noConformeCount,
    puntaje: score,
  ...
  const triggerSync = async (payload: ExamResult) => {
    ...
    try {
      ...
      const googleScriptUrl = "https://script.google.com/macros/s/AKfycbzz71HuKVcZ-nQExfR38Tc5-Z90MHmIJ5VWOV5tJdcXR6gWuoRXyIX0dTDgRLZsTNQ_fg/exec";
      ...
      >
        ...
        <div className="p-6">
          ...
          <div className="flex flex-col items-center justify-center py-4 bg-slate-50 rounded-2xl border border-slate-100 mb-6">
            ...
            <div className="flex flex-wrap justify-center gap-2 mt-3 text-xs font-bold">
              <div className="flex items-center gap-1 bg-emerald-100/70 text-emerald-800 px-2.5 py-1 rounded-xl">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                {conformeCount} Conforme
              </div>
              <div className="flex items-center gap-1 bg-rose-100/70 text-rose-800 px-2.5 py-1 rounded-xl">
                <XCircle className="w-3.5 h-3.5 text-rose-600" />
                {noConformeCount} No conforme
              </div>
              <div className="flex items-center gap-1 bg-slate-100 text-slate-700 px-2.5 py-1 rounded-xl border border-slate-200/60">
                {noAplicaCount} N/A
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3 mb-6 bg-slate-50/80 p-4 rounded-2xl border border-slate-100">
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-500 border-b pb-1.5 flex justify-between items-center">
              <span>Resumen de Evaluación</span>
              <span className="text-blue-600 font-bold font-mono">{bancoUsado}</span>
            </h4>
            
            <div className="space-y-2 text-xs text-slate-700 font-medium">
              <div className="flex justify-between items-center py-0.5 border-b border-slate-200/50">
                <span className="text-slate-500">Vehículo seleccionado:</span>
                <span className="font-bold text-slate-800">{registration.tipoLicencia}</span>
              </div>

              <div className="flex justify-between items-center py-0.5 border-b border-slate-200/50">
                <span className="text-slate-500">Banco de preguntas usado:</span>
                <span className="font-bold text-blue-700">{bancoUsado}</span>
              </div>

              <div className="flex justify-between items-center py-0.5 border-b border-slate-200/50">
                <span className="text-slate-500">Preguntas respondidas como Conforme:</span>
                <span className="font-bold text-emerald-700">{conformeCount}</span>
              </div>

              <div className="flex justify-between items-center py-0.5 border-b border-slate-200/50">
                <span className="text-slate-500">Preguntas respondidas como No conforme:</span>
                <span className="font-bold text-rose-700">{noConformeCount}</span>
              </div>

              <div className="flex justify-between items-center py-0.5 border-b border-slate-200/50">
                <span className="text-slate-500">Preguntas marcadas como No aplica (N/A):</span>
                <span className="font-bold text-slate-600">{noAplicaCount}</span>
              </div>

              <div className="flex justify-between items-center py-0.5 border-b border-slate-200/50">
                <span className="text-slate-500">Total de preguntas evaluadas:</span>
                <span className="font-bold text-slate-900">{totalEvaluadas}</span>
              </div>

              <div className="flex justify-between items-center pt-1.5 text-sm font-extrabold text-blue-900">
                <span>PUNTAJE FINAL:</span>
                <span className="text-base font-black font-mono text-blue-700 bg-blue-100/80 px-2.5 py-0.5 rounded-lg">
                  {score} pts
                </span>
              </div>
            </div>
          ...
          <div className="space-y-3 mb-6">
            ...
            <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-xs text-slate-700">
              ...
              <div className="flex items-start gap-1.5">
                ...
                <div>
                  <p className="text-[10px] text-slate-400 font-semibold uppercase leading-tight">Tipo de Vehículo</p>
                  <p className="font-bold text-slate-800 mt-0.5">{registration.tipoLicencia}</p>
                </div>
              </div>
