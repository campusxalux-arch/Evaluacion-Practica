...
export default function App() {
  ...
  const handleRegistrationSubmit = async (data: UserRegistration) => {
    ...
    setStep("loading");
    setLoadingText("Cargando lista de inspección...");
    setError(null);
    ...
    try {
      ...
      // Select question bank based on Vehicle Type
      const { bankName, questions: selectedQuestions } = getQuestionsForVehicle(data.tipoLicencia);
      setLoadingText(`Cargando ${bankName}...`);
      await delay(400);
      ...
      if (selectedQuestions.length > 0) {
        ...
        throw new Error("No hay preguntas disponibles para el vehículo seleccionado.");
      }
    } catch (err) {
      console.error("Error al iniciar la evaluación:", err);
      let errMsg = err instanceof Error ? err.message : "Fallo al cargar la lista de inspección. Por favor reintente.";
      setError(errMsg);
              ...
              >
                ...
                <div className="text-center pt-1 pb-1">
                  ...
                  <h1 className="text-lg font-black text-slate-900 tracking-tight uppercase leading-snug">
                    Evaluación <span className="text-blue-600">Práctica</span>
                  </h1>
                </div>
